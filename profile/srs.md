...
| 6.4.1 |  |  | 문서별 위험도 점수와 상태(정상/의심/위조)를 포함한다. | 분석된 점수 리스트<br>분석 결과(텍스트, 레이아웃, 직인) | CASE 1: 최종 상태 '정상'<br>CASE 2: 최종 상태 '의심'<br>CASE 3: 최종 상태 '위조' | CASE 1: 합산 로직 오류(점수 누락)<br>CASE 2: 최종 상태 판별 불가<br>CASE 3: 비정상적인 점수 범위 입력 | 2.1.4 2.2.3 2.3.5 | L |  |
