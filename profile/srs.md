...
| 2.2.1 |  |  | 시스템은 OCR을 통해 추출한 텍스트의 위치, 크기 등 레이아웃 정보를 입력받는다. |  |  |  |  |  |  |
| 2.2.2 |  |  | 입력된 레이아웃 정보를 기반으로 사전에 정의된 4가지 배치 규칙(줄 간격, 높이, 정렬, 단어 간격)의 일관성을 검사하여 점수를 계산한다. |  |  |  |  |  |  |
| 2.2.3 |  |  | 최종 점수에 따라 문서의 위험도를 3단계로 판정하여 결과를 반환한다. |  |  |  |  |  |  |
