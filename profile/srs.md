...
| 1.1.1 |  |  | 사전에 사용자 동의를 받은 통화 녹음본 또는 통화 중 실시간 업로드된 오디오를 앱에서 스프링 서버로 WebSocket을 통해 전송한다. |  |  |  |  |  |  |
| 1.1.2 |  |  | 스프링 서버에서 전달받은 음성을 gRPC를 이용해 Naver CLOVA Speech로 전송하고, CLOVA Speech는 이를 STT로 변환하여 전사 텍스트(대화자 분리, 타임스탬프, 구두점 포함)를 스프링 서버로 반환한다. 스프링 서버는 이 전사 결과를 앱으로 WebSocket을 통해 전달하고, 동시에 탐지 모델(파이썬 서버)에 메시지 큐로 전송한다. |  |  |  |  |  |  |
| 1.2  | 보이스피싱 의심 패턴 분석 | STT로 변환한 텍스트에서 보이스피싱 의심 대화 맥락을 판단한다. |  |  |  |  | 6.5 |  |  |
| 1.2.1 |  |  | 탐지 모델로 전달된 전사 텍스트에서 핵심 의심 키워드를 인식 및 검출한다. |  |  |  | 6.5.1 |  |  |
| 1.2.2 |  |  | 탐지 모델은 검출한 의심 항목에 사전 학습된 위험도 가중치를 부여하고, 반복 발생 시 위험도 점수를 가산한다. |  |  |  | 6.5.2 |  |  |
| 1.2.3 |  |  | 최종 위험도 점수를 파이썬 서버에서 실행한 탐지 모델의 결과로 반환하고, 이를 스프링 서버로 전달한다. |  |  |  | 6.5.3 |  |  |
| 1.2.4 |  |  | 결과를 받은 스프링 서버는 위험도 레벨에 따른 결과를 앱에 WebSocket으로 전송한다. |  |  |  |  |  |  |
| 1.3  | 의심 경고 | 위험도가 특정 기준을 초과하면 경고 메시지를 표시한다. |  |  |  |  |  |  |  |
| 1.3.1 |  |  | 위험도가 특정 기준을 초과하면, 스프링 서버는 앱 화면에 보이스피싱 경고 메시지를 표시한다. |  |  |  |  |  |  |
| 2    | 문서 위조 판별 |  |  |  |  |  |  |  |  |
| 2.1  | 의심 키워드 판별 | OCR을 통해 추출한 문서 내 텍스트를 기반으로 위조 의심 여부를 판단한다. |  |  |  |  |  |  |  |
| 2.1.1  |  |  | 시스템은 사용자에게 이미지 파일을 입력받는다. |  |  |  | 6.1 |  |  |
| 2.1.2  |  |  | OCR API를 호출하여 이미지에서 텍스트를 추출해 JSON 파일을 만든다. |  |  |  | 6.2 |  |  |
| 2.1.3  |  |  | JSON을 탐색하여 미리 설정해둔 의심 키워드가 감지되면 해당 키워드의 등급에 따른 점수를 더한다. |  |  |  | 6.3.1 |  |  |
| 2.1.4  |  |  | 최종 점수에 따라 문서의 위험도를 3단계로 판정하여 결과를 반환한다. |  |  |  | 6.4.1 |  |  |
...
| 6.6.1 |  |  | 메시지 텍스트를 입력받아 3.2.1에 따라 정규화하여 처리한다. |  |  |  | 3.2.1 |  |  |
| 6.6.2 |  |  | 3.2.2에 따라 키워드 대조 및 위험도 등급을 부여한다. |  |  |  | 3.2.2 |  |  |
| 6.6.3 |  |  | 3.2.3에 따라 키워드 빈도 및 가중치를 기반으로 최종 위험도 점수를 산정한다. |  |  |  | 3.2.3 |  |  |
| 6.6.4 |  |  | 3.2.4에 따라 위험도 점수를 3단계로 분류하여 최종 판정 결과를 반환한다. |  |  |  | 3.2.4 |  |  |
