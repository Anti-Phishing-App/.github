...
| 2.2.1 |  |  | 시스템은 사용자에게 이미지 파일을 입력받는다. |  |  |  | 6.1 | H |  |
| 2.2.2 |  |  | 시스템은 OCR을 통해 추출한 텍스트의 위치, 크기 등 레이아웃 정보를 입력받는다. |  |  |  | 6.2.3 | M |  |
| 2.2.3 |  |  | 입력된 레이아웃 정보를 기반으로 사전에 정의된 4가지 배치 규칙(줄 간격, 높이, 정렬, 단어 간격)의 일관성을 검사하여 점수를 계산한다. |  |  |  | 6.3.2 | M |  |
| 2.2.4 |  |  | 최종 점수에 따라 문서의 위험도를 3단계로 판정하여 결과를 반환한다. |  |  |  | 6.4.1 | L |  |
...
| 3.2.2 |  |  | 전처리된 텍스트를 서버로 전송하여 워험도 점수를 계산한다 |  |  |  | 6.6.1 | L |  |
| 3.2.3 |  |  | 서버로 부터 전달받은 위험도 점수를 기준으로 안전, 주의, 위험 3단계로 분류하여 최종 피싱 의심도를 판정한다. |  |  |  | 6.6.2 | L |  |
