...
| 1.2  | 보이스피싱 의심 패턴 분석 | STT로 변환한 텍스트에서 보이스피싱 의심 대화 맥락을 판단한다. |  |  |  |  |  |  |  |
| 1.2.1 |  |  | 탐지 모델로 전달된 전사 텍스트에서 핵심 의심 키워드를 인식 및 검출한다. |  |  |  |  |  |  |
| 1.2.2 |  |  | 탐지 모델은 검출한 의심 항목에 사전 학습된 위험도 가중치를 부여하고, 반복 발생 시 위험도 점수를 가산한다. |  |  |  |  |  |  |
| 1.2.3 |  |  | 최종 위험도 점수를 산정하고, 해당 결과를 메시지 큐를 통해 스프링 서버로 전달한다. 스프링 서버는 위험도 레벨에 따른 결과를 앱에 WebSocket으로 전송한다. |  |  |  |  |  |  |
