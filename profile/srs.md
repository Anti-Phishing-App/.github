...
| 1.1.1 |  |  | 사전에 사용자 동의를 받은 통화 녹음본 또는 통화 중 실시간 업로드된 오디오를 NCP CLOVA Speech로 전송한다. |  |  |  |  |  |  |
| 1.1.2 |  |  | 대화자 분리(선택), 타임스탬프, 문장 단위 구두점/정확도를 포함한 전사 결과를 수신한다. |  |  |  |  |  |  |
| 1.2  | 보이스피싱 의심 패턴 분석 | STT로 변환한 텍스트에서 보이스피싱 의심 대화 맥락을 판단한다. |  |  |  |  |  |  |  |
| 1.2.1 |  |  | 전사 텍스트에서 핵심 의심 키워드를 인식 및 검출한다. |  |  |  |  |  |  |
| 1.2.2 |  |  | 검출한 의심 항목에 사전 학습된 위험도 가중치를 부여하고, 의심 항목이 계속 반복되면 위험도 점수를 가산한다. |  |  |  |  |  |  |
| 1.2.3 |  |  | 최종 위험도 점수를 산정하고, 임계치에 따라 위험도를 구분한다. |  |  |  |  |  |  |
| 1.3  | 의심 경고 | 위험도가 특정 기준을 초과하면 경고 메시지를 표시한다. |  |  |  |  |  |  |  |
| 1.3.1 |  |  | 위험도가 특정 기준을 초과하면 화면에 보이스피싱 경고 메시지를 표시한다. |  |  |  |  |  |  |
| 2    | 문서 위조 판별 |  |  |  |  |  |  |  |  |
