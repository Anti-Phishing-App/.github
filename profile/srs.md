...
| 1.2  | 실시간 보이스피싱 탐지 | 통화 중 음성을 실시간으로 분석하여 위험 여부를 즉시 판단한다. | 앱 환경에서는 실시간 통화 시 마이크 권한 또는 오디오 입력 문제로 인해 음성 신호가 정상적으로 캡처되지 않아 volume 값이 0으로 전달되는 현상이 발생하였으며, 이에 따라 웹 환경에서 동일한 실시간 보이스피싱 분석 로직을 적용하여 테스트를 진행한 결과 정상적으로 동작함을 확인 |  |  |  |  |  |세부요구사항 설명 참고바랍니다.|
